...
require '../header.php';
require '../db_api.php';
$date = isset($_GET["time"]) ? $_GET["time"] : date("Y-m-d", time());
$result = drive_sleepy($date);
$limit = 5;
$count = $result["count"];
...
<?php foreach ($result[0] as $item) {?>
                            <tr>
                                ...
                                <td><?php echo $item["时长"];?></td>
                                <td><a href="../car/map.php?id=<?php echo $item['车牌号'];?>&time=<?php echo $item["时间"];?>">详情（地图）</a></td>
                            </tr>
                        <?php }
?>
                        </table>
